...
const Header = React.memo(({ title, showBack = false, onBack }) => (
...
const Navigation = React.memo(({ currentScreen, onScreenChange }) => (
...
const HomeScreen = React.memo(({ onResourceClick, onEmergencyCall }) => (
  <div className="flex flex-col h-screen">
    <Header title="Emergency Resource Finder" />

    <div className="flex-1 flex flex-col pb-20 overflow-hidden p-3"> {/* p-3 pt-1 */}
      <div className="grid grid-cols-2 gap-4 px-2 py-2 flex-1 min-h-0">
        {resourceCategories.map((resource, index) => (
          ...
          >
            <div className="relative w-full bg-center bg-no-repeat bg-cover rounded-lg flex-1 min-h-0 overflow-hidden">
              <div
                className="w-full h-full bg-center bg-no-repeat bg-cover"
                style={{ backgroundImage: `url(${resource.image})` }}
              />
              <div className="absolute inset-0 bg-[#6ee7b7] bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 rounded-lg" />
            </div>
        ...
        >
          ...
          {selectedPlace && (
            <InfoWindow
              ...
              onCloseClick={onSelectedPlaceClose}
            >
...
const ContactsScreen = React.memo(({ 
  ...
  if (contactsSubScreen === "add") {
    return (
      <div className="flex flex-col h-screen">
        <Header title="Add Contact" showBack onBack={() => onSubScreenChange("hotlines")} />
  ...
  if (contactsSubScreen === "your") {
    return (
      <div className="flex flex-col h-screen">
        <Header title="Your Contacts" showBack onBack={() => onSubScreenChange("hotlines")} />
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 pb-16 overflow-y-auto">
        <div className="flex justify-center gap-4 p-4 border-b border-[#2c3135]">
          ...
          <button
            onClick={() => onSubScreenChange("your")}
            className="flex items-center gap-2 bg-[#2c3135] px-4 py-2 rounded-full text-white hover:bg-[#3c4147] transition-colors"
          >
            <Users size={18} />
            Your Contacts
...
export default function App() {
  ...
  // Contacts state using custom hook
...
// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleScreenChange = useCallback((screen) => {
    ...
    if (screen === "contacts") setContactsSubScreen("hotlines");
  }, []);
  ...
  const handleEmergencyCall = useCallback((number = "911") => {
    window.open(`tel:${formatPhoneDigits(number)}`);
  }, []);
  ...
  const fetchPlaceDetails = useCallback((place) => {
    if (!isOnline || !mapRef.current) {
      setSelectedPlace({
        ...place,
        displayName: place.displayName || place.name || 'Unknown Location'
      });
      return;
    }
  ...
  const handleSelectedPlaceClose = useCallback(() => {
    setSelectedPlace(null);
  }, []);

  const handleSubScreenChange = useCallback((subScreen) => {
    setContactsSubScreen(subScreen);
  }, []);

  const handleNewContactChange = useCallback((contact) => {
  ...
  }, [newContact.name, newContact.phone, setContacts]);
  ...
  const renderCurrentScreen = () => {
    switch (currentScreen) {
      ...
      case "map":
        return (
          <MapScreen 
            ...
            places={places}
            offlineFiltered={offlineFiltered}
            isLoaded={isLoaded}
            userLocation={userLocation}
            onMapLoad={handleMapLoad}
            fetchPlaceDetails={fetchPlaceDetails}
            selectedPlace={selectedPlace}
            onSelectedPlaceClose={handleSelectedPlaceClose}
      ...
      case "contacts":
        return (
          <ContactsScreen 
            contactsSubScreen={contactsSubScreen}
